...
gsl_matrix* getTestMicPos(){
    micPos=gsl_matrix_alloc(4,3);
    for(int i=0;i<4;i++){
        gsl_matrix_set(micPos, i, 0, i*0.05-0.075);
        gsl_matrix_set(micPos, i, 1, 0);
        gsl_matrix_set(micPos, i, 2, 0);
    }
    return micPos;
}
 ...
 */
bool doBeamforming(const float** inputSig, const char *coeffFile, 
		    float azimuth, float elevation,
		    double sampleRate, unsigned fftLen, int pf, double alpha, 
		    unsigned D, unsigned m, unsigned r, const float* outputSig)
{
  ...
  ZelinskiPostFilterPtr output = new ZelinskiPostFilter((VectorComplexFeatureStreamPtr&)beamformerP, 
    fftLen, alpha, pf );
  OverSampledDFTSynthesisBankPtr synthesisFBP = new OverSampledDFTSynthesisBank(
    (VectorComplexFeatureStreamPtr&)output, g_fb, fftLen, m, r );

  gsl_matrix* micPos = getTestMicPos();
  ...
  while(1){
    ...
    OverSampledDFTAnalysisBankPtr analysisFBP = new OverSampledDFTAnalysisBank(
        (VectorFloatFeatureStreamPtr&)sampleFeatureP, h_fb, fftLen, m, r );
